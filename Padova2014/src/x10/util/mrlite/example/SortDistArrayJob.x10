...
implements Job[Long,Long,Long,Pair[Long, Long],Long,Rail[Pair[Long,Long]]] {
	...
	var keyRail:GlobalRef[Rail[Long]] = GlobalRef[Rail[Long]](new Rail[Long](Place.MAX_PLACES));
	var hashM:GlobalRef[HashMap[Long, Rail[Pair[Long,Long]]]] = GlobalRef[HashMap[Long, Rail[Pair[Long,Long]]]](new HashMap[Long, Rail[Pair[Long,Long]]](Place.MAX_PLACES));
	...
	public def mapper(k:Long, v:Long, s:(Long,Pair[Long, Long])=>void):void {
		val span = max-min;
		s(v / (span/ (Place.MAX_PLACES-1)), Pair[Long, Long](v,k));		
	}
	...
	public static def test0(args:Rail[String]) {
		...
		try {
			Console.OUT.print("{");
			finish for (p in PlaceGroup.WORLD) at(p) async {
		...
		job.max = finish(Reducible.MaxReducer[Long](Long.MIN_VALUE)) {
			async for(p in PlaceGroup.WORLD) at(p) {
