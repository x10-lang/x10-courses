...
public class Stream[T](s:()=>Cons[T]) implements ()=>Cons[T] {
    ...
    */
     ...
     */
    public static def append[T](x:Rail[T], f:Stream[T]):Stream[T]=append(x, 0,f);
    static def append[T](x:Rail[T], i:Int,f:Stream[T]):Stream[T]=
        x(i) ~ (i<x.size? append[T](x, i+1,f):f);
    ...
    public static def append(V:IntRange,f:Stream[Int]):Stream[Int]=
        V.min ~ (V.min==V.max ? f : append((V.min+1)..V.max,f));
    ...
    */
     ...
     */
    public static def nats(i:Int):Stream[Int]=gen[Int](i, (n:Int)=>n+1);
    public static val nats:Stream[Int]=nats(0);
